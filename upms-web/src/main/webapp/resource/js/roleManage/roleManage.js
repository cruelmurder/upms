		...
		columns : [[{
			...
			formatter : function(value,rowData,index){
				...
				if(permissable("upms:role:accredit")){ //角色授权权限
					...
					var a = '<a href="javascript:void(0)" style="color:#000; cursor: pointer;" onclick="authorizerow('+srId+','+sysId+')">[菜单授权]</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'
					+'<a href="javascript:void(0)" style="color:#000; cursor: pointer;" onclick="dataAuthorizerow('+srId+','+sysId+')">[数据授权]</a>';
				}
				return a;
			}
		}, 
...
//						showDataButtonViewFunc(node, srId, sysId);
...
*点击“数据授权”后，显示右方数据
*/
function dataAuthorizerow(srId,sysId){ 
	$("#test_layout2").hide();
	/*展开右边  &ouyang*/
	$("#test_layout").layout("expand", "east");
	$("#test_layout").layout("collapse", "east");
	...
	$('#authorize_panel').panel('close');
	$('#data_layout').show();
	$('#data_panel').panel('open');
	/**
	 * 页面上保存选中行的角色ID和系统Id
	 */
	$('#hiddenSrId').val(srId);
	$('#hiddenSysId').val(sysId);
	$('#dataPerview').datagrid({//打开并显示按钮列表
		//title : '按钮功能',
		...
		method : 'post',
		//border: false,
		url : '../dataPermissionManage/dataPermissionList.json?srId='+srId+'&sysId='+sysId,
		frozenColumns:[[
		...
		onLoadSuccess:function(data){  
			if(data){
	            $.each(data.rows, function(index, item){
			      if(item.isChecked){
			         $('#dataPerview').datagrid('checkRow', index);
			      }
			    });
			 }
		}        
...
function makeButtonPermission(){
	...
	function(data){
	    var opResult = data.success;
	     if(opResult==true){
	    	 msg.bottomRight("提示信息", "按钮权限保存成功");
	    	
	     }else{
	    	 var msgStr = data.msg;
	    	 msg.bottomRight("失败信息", "按钮权限保存失败!"+msgStr);
...
*为角色授权操作,针对角色的数据权限授权
*/
function makeDataPermission(){
	//已选择的数据权限行
	var dataRows = $('#dataPerview').datagrid('getSelections');
	var dataStr ='';
	for(var i=0;i<dataRows.length;i++){
		if (dataStr != '') dataStr += ',';
		dataStr += dataRows[i].id;
	}
	
	var row_sysId = $("#hiddenSysId").val();
	var row_srId = $("#hiddenSrId").val();
	
	$.post("../rolepermission/makeDataPermission.json",
			{'sysId':row_sysId,'srId':row_srId,"dataStr":dataStr},
	function(data){
	    var opResult = data.success;
	     if(opResult==true){
	    	 msg.bottomRight("提示信息", "数据权限保存成功");
	    	
	     }else{
	    	 var msgStr = data.msg;
	    	 msg.bottomRight("失败信息", "数据权限保存失败!"+msgStr);
