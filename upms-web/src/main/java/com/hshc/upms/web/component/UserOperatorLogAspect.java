 ...
 * 
 */
package com.hshc.upms.web.component;
...
import com.hshc.common.util.IpUtils;
...
public class UserOperatorLogAspect {
	...
	@Autowired
	private OperationLogService<OperationLog, Long> operationLogService;
	...
	private void saveOperator(JoinPoint joinPoint, Object rtv) throws Exception {
		...
		if (user != null) {
			...
			if (!className.equalsIgnoreCase("LoginSuccessLog")
				...
				OperationLog operatorLog = new OperationLog(title,
						user.getAccount(), opContent,
						IpUtils.getIpAddr(request), new Date());
				this.operationLogService.createEntity(operatorLog);
			}
		}
	}
